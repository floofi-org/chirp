...
if (!$loggedIn) {
    header("Location: /");
    die();
} elseif (!in_array($profile["id"], json_decode(file_get_contents($_SERVER['DOCUMENT_ROOT'] . "/includes/tokens.json"), true)['oauth']['banned'])) {
    header("Location: /app");
    die();
}
...
<main class="container" style="background-color: #9c1d96; color: white; border-radius: 30px; padding: 30px;">
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; grid-gap: 30px;" id="panes">
        <div id="pane-right" style="order: 2;">
            <h2 style="margin-bottom: 30px;">You are banned from using Sunny Starbot</h2>

            <div class="alert alert-warning">
                Your behavior was problematic (for example, you didn't respect the terms of use), and the administrators have decided to prevent you from using Sunny Starbot any further. If you wish to appeal this decision, <a href="https://equestria.dev/contact" target="_blank" style="color: white;">contact us</a>.
            </div>
